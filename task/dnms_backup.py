...
import pdb
class DMC():
    def __init__(self, N=750, mean=0.5, var=0.1):
        self.N = N
        self._mean = mean
        self._var = var
        self._version = ""
    ...
    def GetInput(self, mean_overide=-1, var_overide=1):
        ...
        if torch.rand(1).item() < 0.5:   # channel one is active
            inpts[:200,0] = torch.ones(200)
            sample_channel=0
        else:                            # signal is negative
            inpts[:200,1] = torch.ones(200)
            sample_channel=1
        ...
        if torch.rand(1).item() < 0.5:   # channel zero is active
            inpts[400:600,0] = torch.ones(200)
            test_channel=0
        else:                            # channel two is active
            inpts[400:600,1] = torch.ones(200)
            test_channel=1
        # adds noise to sample stimulus
    ...
    def Loss(self, y, target, errorTrigger=-1):
        ...
        if type(y) is np.ndarray:
            ...
            meanSquareLoss = torch.sum( squareLoss, axis=0 ) / 500
