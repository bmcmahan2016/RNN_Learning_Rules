...
int NBNEUR = 50;
int NBIN = 3;  // Input 0 is reserved for an unused 'go' signal
int NBOUT = 1;
...
// some task specific hyper-parameters
double ALPHABIAS = 1; //.1857;
int threshold = 0.75;
int trail_start_time = 200;
...
std::uniform_real_distribution<double> event_rate(0.5, 1.0);
...
int main(int argc, char* argv[])
{
    ...
    int NBTRIALTYPES = 3; // 3 trial types for multisensory integration task
...
int trialtype;
    ...
    int NBTRIALS =  100401; 
    int TRIALTIME = 700;
    int STARTSTIM1 = 200, TIMESTIM1 = 500; 
    ...
    for (int numtrial=0; numtrial < NBTRIALS; numtrial++)
    {
        ...
        if (trialtype == 0){                         // channel 1 is active
            biasmodality1 = event_rate(myrng);
            tgtresp = biasmodality1 < .75 ? 0 : 1.0;
        } 
        else if (trialtype == 1){                    // channel 2 is active
            biasmodality2 = event_rate(myrng);
            tgtresp = biasmodality2 < .75 ? 0 : 1.0;
        } 
        else if (trialtype == 2){                    // congruent channels
            biasmodality1 = event_rate(myrng);
            biasmodality2 = biasmodality1;
            tgtresp = biasmodality1 < .75 ? 0 : 1.0;
        ...
        */
        ...
        for (int numiter=0; numiter < TRIALTIME;  numiter++)
        {
            ...
            if (numiter >= STARTSTIM1)
            {
                ...
                input(1) =  0.05*Gauss(myrng) + biasmodality1;     // biasmodality is just the event rate
                input(2) =  0.05*Gauss(myrng) + biasmodality2;     // Gauss(myrng) is noise drawn from N(0, 1)
       ...
       int EVALTIME = 200; 
        ...
        if (PHASE == LEARNING)
        {
            ...
            if (numtrial == NBTRIALS - 1) // training completed
            {
                saveWeights(J, "J.dat");
                ...
                myfile.open("win" + std::to_string(numtrial) + ".txt", ios::trunc | ios::out);  myfile << win << endl; myfile.close();
